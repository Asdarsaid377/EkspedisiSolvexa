...
// Harga Etalase = harga_katalog × 1.35, dibulatkan ke atas
...
export default function EtalasePage() {
	...
	const load = async () => {
		...
		// Sengaja TANPA .in("produk_id", ids) — dengan banyak produk, URL query
		// .in() jadi sangat panjang (semua UUID digabung di query string) dan bisa
		// kena limit ukuran URI di reverse proxy (muncul sebagai "Error CORS" di
		// browser karena preflight gagal 414, bukan CORS beneran). Ambil semua
		// baris sekalian, sama seperti /katalog.
	...
	const addToCart = (p: Produk) => {
		const harga = hargaEtalase(p.harga_katalog);
		const custom = p.stok <= 0;
		setCart((prev) => {
	...
	return (
		<div className="min-h-screen bg-gray-50">
			...
			<header className="sticky top-0 z-30 bg-white border-b border-gray-100 shadow-sm">
				<div className="max-w-5xl mx-auto px-4 py-3 flex items-center gap-3">
					<div className="flex-shrink-0">
						<p className="text-xs text-gray-400 leading-none flex items-center gap-1">
							<Sparkles size={11} className="text-amber-500" /> Etalase
						</p>
					...
					<div className="relative flex-1">
						...
						<input
							...
							className="w-full pl-9 pr-8 py-2 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-amber-500 bg-gray-50"
						/>
						{search && (
							<button
								onClick={() => setSearch("")}
					...
					<button
						...
						className="relative flex-shrink-0 w-10 h-10 flex items-center justify-center rounded-xl bg-amber-50 hover:bg-amber-100 text-amber-600 transition">
						...
						{filtered.map((p) => {
							...
							return (
								<div
									...
									<div
										onClick={() => router.push(`/etalase/${p.id}`)}
										...
										{inCart ? (
											<div className="mt-2 flex items-center justify-between bg-amber-50 rounded-xl px-2 py-1">
												<button
													...
													className="w-7 h-7 flex items-center justify-center text-amber-600 hover:bg-white rounded-lg transition">
													<Minus size={14} />
												</button>
											...
											<button
												onClick={() => addToCart(p)}
												className="mt-2 w-full py-2 bg-white border-2 border-amber-400 text-amber-600 hover:bg-amber-50 text-xs font-semibold rounded-xl transition flex items-center justify-center gap-1">
												<ClipboardList size={13} /> Pesan Custom/PO
											</button>
										) : (
											<button
												onClick={() => addToCart(p)}
												className="mt-2 w-full py-2 bg-amber-500 hover:bg-amber-600 text-white text-xs font-semibold rounded-xl transition flex items-center justify-center gap-1">
			...
			{totalItem > 0 && !cartOpen && (
				<button
					onClick={() => setCartOpen(true)}
					className="fixed bottom-6 right-4 z-40 flex items-center gap-2 bg-amber-500 hover:bg-amber-600 text-white px-4 py-3 rounded-2xl shadow-lg shadow-amber-200 transition">
