...
export default function LaporanResellerPage() {
	...
	const [stats, setStats] = useState<ResellerStat[]>([]);
	...
	const load = async () => {
		...
		const { data } = await supabase
			.from("penjualan")
			.select(
				"reseller_id, reseller:resellers(nama), total_harga_katalog, total_bonus, items:penjualan_item(jumlah)",
			)
			.gte("tanggal", filter.dari + "T00:00:00")
			.lte("tanggal", filter.sampai + "T23:59:59")
			.not("reseller_id", "is", null);

		const map: Record<string, ResellerStat> = {};
		for (const p of data || []) {
			...
			map[id].total_omset += p.total_harga_katalog || 0;
			map[id].total_bonus += p.total_bonus || 0;
			map[id].total_unit += (p.items || []).reduce(
		...
		<div>
			<div className="mb-8">
				<h1 className="text-2xl font-bold text-gray-900">
					Laporan Top Reseller
				</h1>
				<p className="text-gray-500 mt-1">
					Top 20 reseller berdasarkan omset & unit penjualan
				</p>
			</div>
				...
				<>
					...
					<div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 mb-6">
						<h2 className="text-base font-semibold text-gray-900 mb-5">
							Top {Math.min(10, sorted.length)} Reseller —{" "}
