...
import { Trophy, Truck, Package, MapPin, Award } from "lucide-react";
...
export default function LaporanSopirPage() {
	...
	const [stats, setStats] = useState<SopirStat[]>([]);
	...
	const load = useCallback(async () => {
		...
		const { data } = await supabase
			.from("penjualan")
			.select("sopir, tujuan, penjualan_item(jumlah)")
			.not("sopir", "is", null)
			.neq("sopir", "")
		...
		// Agregasi per sopir
		const map: Record<string, { pengantaran: number; unit: number; tujuan: Record<string, number> }> = {};

		for (const p of data || []) {
			const nama = (p.sopir as string).trim();
			if (!nama) continue;
			if (!map[nama]) map[nama] = { pengantaran: 0, unit: 0, tujuan: {} };
			map[nama].pengantaran++;
			for (const item of (p.penjualan_item as any[]) || []) {
				map[nama].unit += item.jumlah || 0;
			}
			if (p.tujuan) {
				const t = (p.tujuan as string).trim();
				if (t) map[nama].tujuan[t] = (map[nama].tujuan[t] || 0) + 1;
			}
		}

		const result: SopirStat[] = Object.entries(map)
			.map(([nama, d]) => ({
				nama,
				pengantaran: d.pengantaran,
				unit: d.unit,
				rataUnit: d.pengantaran > 0 ? Math.round(d.unit / d.pengantaran) : 0,
				tujuanTerbanyak: Object.entries(d.tujuan).sort((a, b) => b[1] - a[1])[0]?.[0] || "-",
			}))
			.sort((a, b) => b.pengantaran - a.pengantaran);
	...
	return (
		<div>
			...
			<div className="flex items-center justify-between mb-6">
				<div>
					<h1 className="text-2xl font-bold text-gray-900">Laporan Sopir</h1>
					<p className="text-gray-500 mt-1">Peringkat sopir berdasarkan jumlah pengantaran</p>
				</div>
			...
			{loading ? (
				...
				<div className="text-center py-20">
					...
					<p className="text-gray-500 font-medium">Tidak ada data sopir di periode ini</p>
					<p className="text-sm text-gray-400 mt-1">Pastikan field sopir diisi saat input penjualan</p>
				</div>
			) : (
				<div className="space-y-6">
					...
					{top3.length > 0 && (
						<div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
							<div className="flex items-center gap-2 mb-6">
								...
								<h2 className="font-semibold text-gray-800">Top 3 Sopir Terbaik</h2>
							</div>

							{/* Podium visual */}
							<div className="flex items-end justify-center gap-3 mb-6">
								{podiumOrder.map((sopir, i) => {
									if (!sopir) return <div key={i} className="w-36" />;
									const medal = podiumMedal[i];
									const rank = podiumRank[i];
									return (
										<div key={sopir.nama} className="flex flex-col items-center w-36">
											{/* Card di atas podium */}
							...
							<div className="grid grid-cols-3 gap-3">
								{top3.map((s, i) => (
									<div key={s.nama} className="bg-gray-50 rounded-xl p-3 text-center">
										<p className="text-xs text-gray-400 mb-1">Rata-rata / antar</p>
										<p className="font-bold text-gray-900">{s.rataUnit} unit</p>
										{s.tujuanTerbanyak !== "-" && (
											<p className="text-xs text-gray-400 mt-1 flex items-center justify-center gap-1">
												<MapPin size={10} /> {s.tujuanTerbanyak}
											</p>
										)}
									</div>
								))}
					...
					{chartData.length > 0 && (
						<div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
							<h2 className="font-semibold text-gray-800 mb-5">
								Top {chartData.length} Sopir — Jumlah Pengantaran
							</h2>
							<ResponsiveContainer width="100%" height={chartData.length * 44 + 20}>
								<BarChart
									...
									/>
									<Tooltip
										formatter={(value: number) => [`${value} pengantaran`, "Jumlah Antar"]}
										labelFormatter={(label: string, payload: any[]) =>
											payload?.[0]?.payload?.namaFull || label
										}
									/>
									<Bar dataKey="pengantaran" radius={[0, 6, 6, 0]}>
										{chartData.map((_, i) => (
					...
					<div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
						<div className="px-6 py-4 border-b border-gray-100 flex items-center gap-2">
							...
							<span className="ml-auto text-sm text-gray-400">{stats.length} sopir</span>
						</div>
										...
										<tr
											...
											<td className="px-4 py-3.5 text-center">
												<div className="flex flex-col items-center">
													<span className="font-bold text-gray-900 text-base">{s.pengantaran}</span>
													{/* Progress bar relative to top */}
													<div className="w-16 h-1 bg-gray-100 rounded-full mt-1">
														<div
															className="h-full bg-indigo-400 rounded-full"
															style={{ width: `${stats[0] ? (s.pengantaran / stats[0].pengantaran) * 100 : 0}%` }}
														/>
													</div>
												</div>
											</td>
											...
											<td className="px-4 py-3.5 text-center">
												<span className="text-sm text-gray-600">{s.rataUnit}</span>
												<span className="text-xs text-gray-400 ml-1">unit</span>
											</td>
											<td className="px-4 py-3.5">
												{s.tujuanTerbanyak !== "-" ? (
