...
export default function TokoDetailPage() {
	...
	const addToCart = () => {
		if (!produk) return;
		const harga = hargaUmum(produk.harga_katalog);
		setCart((prev) => {
			...
			<div className="sticky top-0 z-30 bg-white border-b border-gray-100 shadow-sm">
				<div className="max-w-2xl mx-auto px-4 py-3 flex items-center justify-between">
					...
					<button
						...
						className="relative flex items-center gap-1.5 text-sm text-indigo-600 font-medium">
						<ShoppingCart size={18} />
						{totalItem > 0 && (
							<span className="absolute -top-2 -right-2 w-4 h-4 bg-indigo-600 text-white text-[9px] font-bold rounded-full flex items-center justify-center">
								{totalItem}
							</span>
						)}
					</button>
			...
			<div className="max-w-2xl mx-auto">
						...
						<div className="flex gap-2 px-4 py-3 overflow-x-auto border-t border-gray-50">
							{fotos.map((url, i) => (
								<button
									key={i}
									onClick={() => setActiveIdx(i)}
									className={`w-14 h-14 flex-shrink-0 rounded-xl overflow-hidden border-2 transition ${i === activeIdx ? "border-indigo-500" : "border-gray-200"}`}>
									<img
										src={imgUrl(url, 120)}
										alt=""
										loading="lazy"
				...
				<div className="bg-white mt-2 px-5 py-5 space-y-4">
					...
					<div className="bg-indigo-50 rounded-2xl px-4 py-4">
						<p className="text-xs text-indigo-500 font-medium mb-1">Harga</p>
						<p className="text-3xl font-bold text-indigo-700">
							{formatRp(harga)}
						</p>
						<p className="text-xs text-indigo-400 mt-1">per {produk.satuan}</p>
					</div>
